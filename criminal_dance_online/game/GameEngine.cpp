...
QString GameEngine::getCurrentPlayerName() const{
    return QString::fromStdString(current_player->get_name());
}

int GameEngine::getCurrentPlayerNumCards() const{
    return current_player->getHandSize();
}

Card::Type GameEngine::getCurrentPlayerHand(const int& index) const{
    return current_player->get_card(index)->get_type();
}
