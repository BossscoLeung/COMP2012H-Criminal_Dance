...
Render::Render(QStringList name_list)
{
    ...
    for (int i = 0; i < num_players; i++) {
        ...
        players[i].setIsFirstPlayer(temp_player->is_first_player());
...
void Render::handles(Card *card, int index)
{
    if (game->getCurrentPlayer()->can_select_card(index)) {
        ...
        players[i].setNumCardChange(-1);
        players[i].setIsInTurn(false);
        players[(i + 1) % game->get_number_of_players()].setIsInTurn(true);
        displayDetails(players[(i + 1) % game->get_number_of_players()].getName(), players[(playerIndex + 1) % game->get_number_of_players()].getNumCard(), players[(playerIndex + 1) % game->get_number_of_players()].getIsAccomplice());
        ...
        game->print_all_player_status();
        qDebug();

        for (int i = 0; i < game->getCurrentPlayerNumCards(); i++) {
            cards[i].setPixmap(QPixmap(QString::fromStdString(game->getCurrentPlayer()->get_card(i)->typeToAddress())).scaled(cards[i].getWidth(), cards[i].getHeight()));
        }
        game->nextPlayer();
